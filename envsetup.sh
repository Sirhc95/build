...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/cypher/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/cypher/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/cypher/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cypher_") ; then
       CYPHER_BUILD=$(echo -n $1 | sed -e 's/^cypher_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $CYPHER_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CYPHER_BUILD=
    fi
    export CYPHER_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${CYPHER_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Cypher github
        T=$(gettop)
        pushd $T > /dev/null
        vendor/cypher/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        vendor/cypher/build/tools/roomservice.py $product true
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/cypher/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. vendor/cypher/build/envsetup.sh
